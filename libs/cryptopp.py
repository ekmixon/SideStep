...
def compileCryptoPP(path_delim, sourceDir, vsPath, sdkPathIncl, kitPathIncl, winApiIncl, vsToolsPath):
# Create lists to store CryptoPP file names
  ...
  pchOptions = '/c /Zi /nologo /W3 /WX- /O2 /Ob2 /Oi /Oy /GL /D NDEBUG /D _WINDOWS /D USE_PRECOMPILED_HEADERS /D WIN32 /D _VC80_UPGRADE=0x0710 /GF /Gm- /EHsc /MT /GS /Gy /fp:precise /Zc:wchar_t /Zc:forScope /Zc:inline /Yc"pch.h" /Fo"' + sourceDir + path_delim + path_delim + '" /Fp"' + sourceDir + path_delim + 'cryptlib.pch" /Gd /TP /analyze- /errorReport:prompt /I"' + vsPath + path_delim + 'include" /I"' + vsPath + path_delim + 'atlmfc' + path_delim + 'include" /I"' + sdkPathIncl + '" /I"' + kitPathIncl + '" /I"' + sourceDir + '" ' + sourceDir + path_delim + 'pch.cpp'

  dll_and_iterhash_Options = '/c /Zi /nologo /W3 /WX- /O2 /Ob2 /Oi /Oy /GL /D NDEBUG /D _WINDOWS /D USE_PRECOMPILED_HEADERS /D WIN32 /D _VC80_UPGRADE=0x0710 /GF /Gm- /EHsc /MT /GS /Gy /fp:precise /Zc:wchar_t /Zc:forScope /Zc:inline /Gd /TP /analyze- /errorReport:prompt /Fo"' + sourceDir + path_delim + path_delim + '" /I"' + sourceDir + '" /I"' + vsPath + path_delim + 'include" /I"' + vsPath + path_delim + 'atlmfc' + path_delim + 'include" /I"' + sdkPathIncl + '" /I"' + kitPathIncl + '" ' + sourceDir + path_delim + 'dll.cpp ' + sourceDir + path_delim + 'iterhash.cpp'

  libOptions = '/OUT:"' + sourceDir + path_delim + 'cryptlib.lib" /NOLOGO /LTCG /MACHINE:X86 ' + cryptoObj

  # Compile pch
  subprocess.Popen(vsToolsPath + path_delim + 'cl.exe ' + pchOptions, stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE)
  time.sleep(5)

  # Compile dll.cpp and iterhash.cpp
  subprocess.Popen(vsToolsPath + path_delim + 'cl.exe ' + dll_and_iterhash_Options, stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE)
  time.sleep(5)

  for item in cryptoCpp:
    # Compile various cpp files to objects
    subprocess.Popen(vsToolsPath + path_delim + 'cl.exe ' + item, stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE)
    time.sleep(7)

  # Create a library from all the object files
  subprocess.Popen(vsToolsPath + path_delim + 'lib.exe ' + libOptions, stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE)
  time.sleep(5)